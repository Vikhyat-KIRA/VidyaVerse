...
export default function CommunityPanel({ userUid, userName }: CommunityPanelProps) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    setMessageInput('');
    await sendMessage(activeRoom.id, text, userUid, userName);
  };
              ...
              {rooms.filter(r => r.type !== 'dm').map(room => (
                ...
                >
                  <div
                    ...
                    style={{ background: room.type === 'auto' ? 'rgba(108, 99, 255, 0.2)' : 'rgba(0, 240, 255, 0.2)' }}
                  >
                    {room.type === 'auto' ? <ShieldCheck size={16} color="#6c63ff" /> : <Hash size={16} color="#00f0ff" />}
                  </div>
                  <div className="flex-1 overflow-hidden">
                    <p className="text-sm font-semibold truncate text-[var(--foreground)]">{room.name}</p>
                    ...
                    return (
                      <div key={msg.id || idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[80%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                          {!isMe && <span className="text-[10px] text-[var(--muted)] ml-1 mb-1">{msg.senderName}</span>}
                          <div
